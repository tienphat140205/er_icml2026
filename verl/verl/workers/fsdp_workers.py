...
class RouterWorker(Worker):
    """
    Router worker for text classification with KL divergence training
    """
    ...
    def _build_router_model_optimizer(self, config):
        from torch import optim
        from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
        from torch.distributed.fsdp import MixedPrecision
        ...
        trust_remote_code = config.model.get("trust_remote_code", False)
        router_model_config = AutoConfig.from_pretrained(
            local_path, trust_remote_code=trust_remote_code
        )
        router_model_config.num_labels = 4  # For the 4 classes: ANSWER, COT, LONG_COT, CODE

        init_context = get_init_weight_context_manager()
        with init_context(), warnings.catch_warnings():
            warnings.simplefilter("ignore")
            setattr(router_model_config, "classifier_dropout", 0.0)  # need more testing here
            setattr(router_model_config, "hidden_dropout", "0")
            router_module = AutoModelForSequenceClassification.from_pretrained(
                pretrained_model_name_or_path=local_path,
                torch_dtype=torch_dtype,
            ...
            # some parameters may not in torch_dtype
            router_module.to(torch_dtype)

            if config.model.get("enable_gradient_checkpointing", False):
                router_module.gradient_checkpointing_enable(
                    gradient_checkpointing_kwargs={"use_reentrant": False}
                )
        if self.rank == 0:
            print_model_size(router_module)
        ...
        auto_wrap_policy = get_fsdp_wrap_policy(
            module=router_module, config=self.config.model.fsdp_config.wrap_policy
        )

        log_gpu_memory_usage("Before router FSDP", logger=None)
        ...
        log_gpu_memory_usage("After router FSDP", logger=None)
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    p
                    for n, p in router_module.named_parameters()
        ...
        router_lr_scheduler = get_constant_schedule_with_warmup(
            optimizer=router_optimizer, num_warmup_steps=num_warmup_steps
        )

        return router_module, router_optimizer, router_lr_scheduler
    ...
    def init_model(self):
        ...
        from verl.workers.router import DataParallelRouter

        self.router_module, self.router_optimizer, self.router_lr_scheduler = (
            self._build_router_model_optimizer(self.config)
        )

        if self._is_offload_param:
            offload_fsdp_param_and_grad(
                module=self.router_module, offload_grad=self._is_offload_grad
            )
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.router_optimizer)

        self.router = DataParallelRouter(
            config=self.config,
            router_module=self.router_module,
            router_optimizer=self.router_optimizer,
        )

        self.flops_counter = FlopsCounter(self.router_model_config)
        self.checkpoint_manager = FSDPCheckpointManager(
            model=self.router_module,
            optimizer=self.router_optimizer,
            lr_scheduler=self.router_lr_scheduler,
            tokenizer=self.tokenizer,
        )
    ...
    def forward(self, data: DataProto):
        ...
        raw_prompts = data.non_tensor_batch["raw_prompt"]  # np.ndarray dtype=object
        texts = [
            p[-1]["content"] if isinstance(p, list) else p["content"] for p in raw_prompts.tolist()
        ]
        ...
        with torch.no_grad():
            logprobs = self.router.compute_router_score(data, micro_batch_size)
            outputs = torch.exp(logprobs)

        output = DataProto.from_dict(tensors={"router_probs": outputs}).to("cpu")
    ...
    def update_router(self, data: DataProto):
        ...
        if self._is_offload_optimizer:
            load_fsdp_optimizer(
                optimizer=self.router_optimizer, device_id=torch.cuda.current_device()
            )
        ...
        # perform training
        with Timer(name="update_router", logger=None) as timer:
            metrics = self.router.update_router(data)
        delta_time = timer.last

        self.router_lr_scheduler.step()
        lr = self.router_lr_scheduler.get_last_lr()[0]
        metrics["router/lr"] = lr
